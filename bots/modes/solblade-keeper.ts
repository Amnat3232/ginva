...
export class SolbladeKeeper {
  ...
  async initialize(): Promise<void> {
    if (!this.config.solbladeMcpCommand) {
      throw new Error(
        "Solblade MCP command is required for Secure Scoped Mode"
      );
    }
    ...
    console.log(
      `💰 Spend limit per tx: ${this.config.spendLimitPerTx || "unlimited"} SOL`
    );
    console.log(
      `👤 Human confirmation: ${
        this.config.requireHumanConfirmation ? "enabled" : "disabled"
      }`
    );
  }
  ...
  async checkLiquidation(): Promise<any[]> {
    ...
    try {
      ...
      const response: any = await this.mcpClient.callTool(
        "check_liquidation_opportunities",
        {
  ...
  async triggerLiquidation(loanPubkey: PublicKey): Promise<string> {
    ...
    try {
      console.log(
        `⚡ Triggering liquidation for loan ${loanPubkey.toBase58()} via Solblade...`
      );
  ...
  async executeStorefrontBuy(
    ...
    try {
      console.log(
        `🛒 Executing storefront buy for collateral ${collateralPubkey.toBase58()} at ${discountPercent}% discount via Solblade...`
      );
