...
export class FullAccessKeeper {
  ...
  async initialize(): Promise<void> {
    console.log(
      `🔑 Full Access Keeper initialized with wallet: ${this.wallet.publicKey.toBase58()}`
    );
  ...
  async triggerLiquidation(loanPubkey: PublicKey): Promise<string> {
    try {
      console.log(
        `⚡ Triggering liquidation for loan ${loanPubkey.toBase58()}...`
      );
  ...
  async executeStorefrontBuy(
    ...
    try {
      console.log(
        `🛒 Executing storefront buy for collateral ${collateralPubkey.toBase58()} at ${discountPercent}% discount...`
      );
